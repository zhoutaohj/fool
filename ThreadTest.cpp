...
int main(int argc, char *argv[])
{
    ...
    fool::Thread producer([&mutex, &task_queue, &cond_full,&cond_empty]() {
        size_t index = 0;
        while (index != 1000000)
        {
    ...
    for (int i = 0; i < 10; i++)
    {
        consumer_vec.push_back(fool::Thread([&mutex, &task_queue, &cond_full,&cond_empty,i]() {
                ...
                while (true )
                {
                    ...
                    if(task_tmp.task_id>=100000&&task_tmp.end =='1')
                    {
                        break;
